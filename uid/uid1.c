...
static struct pam_conv conv = {
    misc_conv, /* Определенная в pam_misc.h conversation-функция*/
    NULL /* На текущий момент, дополнительные данные не требуются*/
};
...
int main()
{
	...
	retval = pam_start(service_name, NULL, &conv, &handle); /* Инициализация PAM */
        if (retval != PAM_SUCCESS){
                fprintf(stderr, "Ошибка инициализации PAM: %s\n", pam_strerror(handle, retval));
                return 1;
        }

        retval = pam_authenticate(handle, 0); /* Выполнить аутентфикацию (будет запрошено имя пользователя и пароль)*/
        if (retval != PAM_SUCCESS) {
